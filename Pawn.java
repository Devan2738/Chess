...
public class Pawn implements ChessPiece{
	...
	/**
	 * This is the constructor for Pawn objects.
	...
	public ChessPiece copyOf() {
		Pawn copy = new Pawn(color, startRow, startColumn);
		return copy;
	}
	...
	public void generateGoodMoves(ChessBoard board) {
		...
		if (color.equals("white")) {
			if (startRow-1 > -1 && startColumn-1 > -1) {
			   ...
			   else if (board.pawnWasMovedTwoSpaces() == true) {
			      if (startRow-1 == board.getEnPassentRow() && startColumn-1 == board.getEnPassentColumn())
			         maybeGoodMoves.add(new MoveCommand(startRow, startColumn, startRow-1, startColumn-1));
			   }
			}

			if (startRow-1 > -1) {  
		    	if (board.get(startRow-1, startColumn).getChar() == '-') {
			         maybeGoodMoves.add(new MoveCommand(startRow, startColumn, startRow-1, startColumn));		    		
		    	}
		    }
		    if (startRow == 6) {
		    	if (board.get(startRow-2, startColumn).getChar() == '-') {
		    		if (board.get(startRow-1, startColumn).getChar() == '-') 
		    		    maybeGoodMoves.add(new MoveCommand(startRow, startColumn, startRow-2, startColumn));
		    	}
		    }
	...
	public void generateNoLossMoves(ChessBoard board) {
		...
		if (color.equals("white")) {
		    ...
		    if (startRow == 6) {
		    	if (board.get(startRow-2, startColumn).getChar() == '-') {
		    		if (board.get(startRow-1, startColumn).getChar() == '-') 
		    		    maybeNoLossMoves.add(new MoveCommand(startRow, startColumn, startRow-2, startColumn));
		    	}
		    }
	...
	public void generateTakeOpponentPieceMoves(ChessBoard board) {
		...
		if (color.equals("white")) {
			if (startRow-1 > -1 && startColumn-1 > -1) {
			   if ((board.get(startRow-1, startColumn-1).getColor().equals("black"))) {
			      takeOpponentPieceMoves.add(new MoveCommand(startRow, startColumn, startRow-1, startColumn-1));
			   }
			   else if (board.pawnWasMovedTwoSpaces() == true) {
			      if (startRow-1 == board.getEnPassentRow() && startColumn-1 == board.getEnPassentColumn())
			         takeOpponentPieceMoves.add(new MoveCommand(startRow, startColumn, startRow-1, startColumn-1));
