...
public class ChessBoard {
	...
	public ChessBoard copyOf() {
		...
		return new ChessBoard(chessPieceCopies, playerWhite.copyOf(), playerBlack.copyOf());
	}
	 ...
	 * @param playerBlack
	 */
	ChessBoard (ChessPiece[][] temp, Player playerWhite, Player playerBlack) {
		this.playerWhite = playerWhite;
		this.playerBlack = playerBlack;	
		board = temp;
	}
	 ...
	 * @return
	 */
	public boolean executeMove(int startRow, int startColumn, int endRow, int endColumn) {
		ChessPiece pieceToMove = get(startRow, startColumn);
		setBoard(startRow, startColumn, new Hyphen());
		char tileTaken = get(endRow, endColumn).getChar();
		...
		if (pieceToMove.getColor().equals("white")) {
			if (get(endRow, endColumn).getChar() == 'K') {
				getPlayerWhite().setTempKingRow(endRow);
				getPlayerWhite().setTempKingColumn(endColumn);		
				if (getPlayerWhite().canDoQSC() == true) {
					if (endRow == 7 && endColumn == 2) {
						setBoard(7, 0, new Hyphen());
						setBoard(7, 3, new Rook("white", 7, 3));
						if (isInCheckState(getPlayerWhite().getTempKingRow(), getPlayerWhite().getTempKingColumn(), "white") == true) {
							return false; 
						}
					}
				}
				else if (getPlayerWhite().canDoKSC() == true) {
					if (endRow == 7 && endColumn == 6) {
						setBoard(7, 7, new Hyphen());
						setBoard(7, 4, new Rook("white", 7, 4));
					}
				}
			}
	    ...
	    if (pieceToMove.getColor().equals("black")) {
			if (get(endRow, endColumn).getChar() == 'k') {
				getPlayerBlack().setTempKingRow(endRow);
				getPlayerBlack().setTempKingColumn(endColumn);		
				if (getPlayerBlack().canDoQSC() == true) {
					if (endRow == 0 && endColumn == 2) {
						setBoard(0, 0, new Hyphen());
						setBoard(0, 3, new Rook("black", 0, 3));
						if (isInCheckState(getPlayerBlack().getTempKingRow(), getPlayerBlack().getTempKingColumn(), "black") == true) {
							return false;
						}
					}
				}
				else if (getPlayerBlack().canDoQSC() == true) {
					if (endRow == 0 && endColumn == 5) {
						setBoard(0, 7, new Hyphen());
						setBoard(0, 4, new Rook("black", 0, 4));
					}
				}
			}
			if (pawnWasMovedTwoSpaces == true) {
